...
public class TileEntityFurnace extends TileEntity implements INamedContainerProvider, ITickableTileEntity {
	...
	private  Object2IntOpenHashMap<Integer> recipes;
  ...
  private FurnaceZoneContents outputZoneContents;
	...
	public TileEntityFurnace(){
	  super(ModTiles.TILE_FURNACE_TUTORIAL.get());
  ...
  // - burn fuel slots
    ...
    BlockState newBlockState = currentBlockState.with(TutorialBlockFurnace.BURNING_SIDES_COUNT, numberBurning);
    ...
    recipes.addTo(new Integer((int) Math.sqrt((double)result.getBurnTime())),1);
  ...
  // This is where you save any data that you don't want to lose when the tile entity unloads
    ...
    outputZoneContents.deserializeNBT(inventoryNBT);
		CompoundNBT compoundnbt = nbtTagCompound.getCompound("RecipesUsed");
    if (fuelZoneContents.getSizeInventory() != FUEL_SLOTS_COUNT
      ...
      throw new IllegalArgumentException("Corrupted NBT: Number of inventory slots did not match expected.");
		...
		while(var4.hasNext()) {
			...
			this.recipes.put(new Integer(s), compoundnbt.getInt(s));
		}
	}
...
//	//  it uses getUpdatePacket(), getUpdateTag(), onDataPacket(), and handleUpdateTag() to do this
  ...
  private ItemStack currentlySmeltingItemLastTick = ItemStack.EMPTY;
	...
	public void grantStoredRecipeExperience(World world, Vector3d pos) {
		...
		ObjectIterator var4 = this.recipes.object2IntEntrySet().iterator();
		while(var4.hasNext()) {
			Object2IntMap.Entry<Integer> entry = (Object2IntMap.Entry)var4.next();

				splitAndSpawnExperience(world, pos, entry.getIntValue(),entry.getIntValue());
