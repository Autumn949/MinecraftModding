...
import com.example.examplemod.inventories.TutorialChestContents;
...
public class ContainerBasic extends Container {
    ...
    public static ContainerBasic createContainerServerSide(int windowID, PlayerInventory playerInventory, TutorialChestContents chestContents) {
    ...
    public static ContainerBasic createContainerClientSide(int windowID, PlayerInventory playerInventory, PacketBuffer extraData) {
        ...
        TutorialChestContents chestContents = TutorialChestContents.createForClientSideContainer(TutorialTileEntity.NUMBER_OF_SLOTS);
    ...
    private static final int NUMBER_OF_SLOT_PR= 9;
    private static final int TE_INVENTORY_FIRST_SLOT_INDEX = VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT;
    private static final int TE_INVENTORY_SLOT_COUNT = TutorialTileEntity.NUMBER_OF_SLOTS;  // must match TileEntityInventoryBasic.NUMBER_OF_SLOTS

    public static final int TILE_INVENTORY_YPOS = 18;  // the ContainerScreenBasic needs to know these so it can tell where to draw the Titles
    public static final int PLAYER_INVENTORY_YPOS = 103;
     ...
     */
    private ContainerBasic(int windowID, PlayerInventory playerInventory, TutorialChestContents chestContents) {
        super(ModTiles.TUTORIAL_CONTAINER.get(), windowID);
        if (ModTiles.TUTORIAL_CONTAINER.get() == null)
        ...
        for(int y = 0;y<NUMBER_OF_ROWS;y++) {
            for (int x = 0; x < (TE_INVENTORY_SLOT_COUNT / NUMBER_OF_ROWS); x++) {
                int slotNumber = x+(y*NUMBER_OF_SLOT_PR);
                addSlot(new Slot(chestContents, slotNumber, TILE_INVENTORY_XPOS + SLOT_X_SPACING * x, (TILE_INVENTORY_YPOS+(y*SLOT_Y_SPACING))));
            }
        }
    }
    ...
    private TutorialChestContents chestContents;
